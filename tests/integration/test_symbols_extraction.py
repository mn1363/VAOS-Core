"""Integration test: collect -> clone -> enumerate -> parse -> extract symbols.
...
`StructuralSymbolExtractor` -- the first concrete `SymbolExtractor` implementation -- consumes
real `ParseResult`s produced by the real flow and yields real `ExtractedSymbol` data. This file
...
from src.extractors.symbols.base import ExtractedSymbolKind
...
@pytest.mark.asyncio
async def test_symbols_are_extracted_from_a_real_local_repository(tmp_path: Path) -> None:
    """The Reference Flow, extended with one `extract_symbols` step, produces real
    `ExtractedSymbol` data from a real, local git repository -- one file with a class and a
    method, one file with a free function and a module-level constant."""
    ...
    _run_git("config", "user.email", "symbols-extraction@example.com", cwd=source_repo)
    _run_git("config", "user.name", "Symbols Extraction Test", cwd=source_repo)
    (source_repo / "widget.py").write_text(
        "class Widget:\n    def greet(self):\n        return 'hello'\n", encoding="utf-8"
    )
    ...
    result = await bootstrap(
        config, extra_steps=[clone_step, enumerate_step, parse_step, extract_symbols_step]
    )
    ...
    symbol_results = result.context.require("symbol_results")
    assert len(symbol_results) == 2

    widget_result = next(r for r in symbol_results if r.relative_path == "widget.py")
    assert widget_result.succeeded is True
    assert [(s.kind, s.name, s.qualified_name) for s in widget_result.symbols] == [
        (ExtractedSymbolKind.CLASS, "Widget", "widget.py::Widget"),
        (ExtractedSymbolKind.METHOD, "greet", "widget.py::Widget.greet"),
    ]

    helpers_result = next(r for r in symbol_results if r.relative_path == "helpers.go")
    assert helpers_result.succeeded is True
    assert [(s.kind, s.name, s.qualified_name) for s in helpers_result.symbols] == [
        (ExtractedSymbolKind.FUNCTION, "retry", "helpers.go::retry"),
        (ExtractedSymbolKind.CONSTANT, "MaxRetries", "helpers.go::MaxRetries"),
    ]
