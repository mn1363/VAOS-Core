"""Integration test: collect -> clone -> enumerate -> parse -> extract AST metadata.
...
`StructuralAstExtractor` -- the first concrete `AstExtractor` implementation -- consumes real
`ParseResult`s produced by the real flow and yields real `AstMetadata` data. This file defines
its own local helpers rather than importing `test_reference_flow.py`'s, matching this
...
@pytest.mark.asyncio
async def test_ast_metadata_is_extracted_from_a_real_local_repository(tmp_path: Path) -> None:
    """The Reference Flow, extended with one `extract_ast` step, produces real `AstMetadata`
    from a real, local git repository -- one Python file, one Go file (to also exercise the
    known documented-Go-source-yields-zero-documented-counts behavior end to end)."""
    ...
    _run_git("config", "user.email", "ast-extraction@example.com", cwd=source_repo)
    _run_git("config", "user.name", "AST Extraction Test", cwd=source_repo)
    (source_repo / "greeter.py").write_text(
        ...
        "\n"
        "\n"
        "async def greet_later(name):\n"
        "    return f'hello {name}'\n",
        encoding="utf-8",
    )
    (source_repo / "widget.go").write_text(
        "package main\n"
        "\n"
    ...
    result = await bootstrap(
        config, extra_steps=[clone_step, enumerate_step, parse_step, extract_ast_step]
    )
    ...
    ast_results = result.context.require("ast_results")
    assert len(ast_results) == 2

    greeter_result = next(r for r in ast_results if r.relative_path == "greeter.py")
    assert greeter_result.succeeded is True
    greeter_metadata = greeter_result.metadata
    assert greeter_metadata is not None
    assert greeter_metadata.function_count == 2
    assert greeter_metadata.async_function_count == 1
    assert greeter_metadata.documented_function_count == 1

    widget_result = next(r for r in ast_results if r.relative_path == "widget.go")
    assert widget_result.succeeded is True
    widget_metadata = widget_result.metadata
    assert widget_metadata is not None
    assert widget_metadata.class_count == 1
    assert widget_metadata.function_count == 1
    # The known Go limitation, verified end to end through the real flow: genuinely documented
    # Go source still yields zero documented counts, since GoParser never carries doc comments
    # into ParseResult in the first place.
    assert widget_metadata.documented_class_count == 0
    assert widget_metadata.documented_function_count == 0
